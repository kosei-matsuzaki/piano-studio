...
**ジャンル均衡化（重要な教訓）**: songbook（Wikifonia）を混ぜると生成が洋楽スタンダード様式に寄る。
- **コードTF**: `--genre-cap 2000`（既定）で songbook を絞れば問題なし。データ3倍の恩恵（ppl 2.71→2.40）を維持。
- **メロディTF**: ジャンル埋め込みがあっても分離しきれず、songbook を**どの比率で混ぜても**
  pop のリズム分布が8分に寄った（55%→64-76%）。既定で **songbook は除外**（`--songbook-cap 0`）。
  復活させる実験は `--songbook-cap N` で可能だが、音価分布・モチーフ反復率の前後比較を必ず行うこと。
- 生成側の保険として neural.js に**音価予算**（曲内の8分率>50%で8分にペナルティ）があり、
  データに関係なく「8分だらけの曲」へのリズムロックを防ぐ。
新データ源を足したら生成品質の指標（モチーフ反復率・和声リズム・音価分布）を必ず前後比較すること。
